...
const channels = {
    'ad1': {
        ...
        drm: { 
            'ZWI1YTNhNDhmM2UxOTFhMDBlM2FjMWUyZDQ3MGM0OTE=': decode('MmM4YjcxOTg1NjM1MjdlNTI0ZDY2NjI4YzA5MmVmMWY=') 
        }
    },
    'vsport': {
...
async function init() {
    ...
    if (channels[key]) {
        ...
        // Log hasil dekripsi untuk memverifikasi
        console.log('Decoded URL:', channel.url);
        console.log('Decoded DRM:', channel.drm);

        if (channel.drm.clearkey) {
            console.log('Using clearkey:', channel.drm.clearkey);
            player.configure({
                drm: {
                    clearKeys: {
            ...
            console.log('Using DRM key:', channel.drm);
            player.configure({
                drm: {
